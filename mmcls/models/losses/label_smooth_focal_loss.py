...
class LabelSmoothFocalLoss(nn.Module):
    ...
    def __init__(self,
        ...
        assert len(alpha) == num_classes, "必须指定LabelSmoothFocalLoss的类别数量"  # 每个类别一个权重
    ...
    def forward(self,
        ...
        # focal loss
        pt = F.softmax(cls_score, dim=-1)  # 各个类别的概率
        # focal loss 的两部分, alpha * gamma
        alpha = self.alpha.to(pt.device)
